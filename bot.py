...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "• תמונה של המוצר\n"
        "• שם המוצר בטקסט\n\n"
        "הבוט יחפש ב-Amazon ויחזיר 5 תוצאות מהזול לביקר.",
        parse_mode="Markdown"
    )
...
async def _do_search(loading_msg, product_name: str = None, image_bytes: bytes = None, image_mime: str = None):
    ...
    try:
        ...
        ali_url = f"https://www.amazon.com/s?k={query}&s=price-asc-rank"
        keyboard = [[InlineKeyboardButton("🛒 חפש ב-Amazon", url=ali_url)]]
